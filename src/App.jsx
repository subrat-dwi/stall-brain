...
export default function App() {
  ...
  const selectedStall = useMemo(() => {
    return stalls.find((stall) => stall.id === selectedStallId) ?? null
  }, [selectedStallId])
    ...
    try {
      ...
      setHistory((prev) => {
        const filtered = prev.filter(
          (item) =>
            item.date !== newEntry.date || item.stallId !== newEntry.stallId,
        )
  ...
  const showCTA = Boolean(selectedStall && phase === 'signalsReady')
  ...
  return (
    <div className="relative min-h-screen bg-[#0f0f0f] text-zinc-100">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col px-4 pb-14 pt-8 sm:px-6 lg:px-10">
        <Header />

        <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-zinc-400">
          <span className="rounded-full border border-zinc-800 bg-zinc-950/60 px-4 py-2">
          ...
          <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-2 text-emerald-200">
            Hyperlocal demand scope: Hazratganj, Lucknow
          </span>
        </div>
        ...
        {showCTA ? (
          <div className="mt-8 flex flex-col items-start gap-3">
            <button
              ...
              className="inline-flex items-center gap-3 rounded-full border border-amber-400/60 bg-amber-500/15 px-6 py-3 text-base font-semibold text-amber-100 shadow-[0_0_25px_rgba(245,158,11,0.25)] transition hover:-translate-y-0.5 hover:border-amber-400 hover:bg-amber-500/25"
            >
              Get today&apos;s forecast
              <span className="text-xs text-amber-200/80">
                Powered by Groq + local signals
              </span>
            </button>
          </div>
        ) : null}
        ...
        {showError ? (
          <div className="mt-8 rounded-2xl border border-rose-500/30 bg-rose-500/10 p-6">
            ...
            <button
              type="button"
              onClick={handleForecast}
              className="mt-4 inline-flex items-center gap-2 rounded-full border border-rose-400/50 bg-rose-500/10 px-4 py-2 text-sm font-semibold text-rose-100 transition hover:border-rose-300"
            >
              {errorDetails.action}
            </button>
          </div>
